...
ggplot(data=CN_heat,aes(phenotype1, phenotype2, fill=groups)) + 
    geom_tile() +
    theme_bw() +
    ggtitle("p value of wilcox test results in CN whinin H haplogrup") +
...
#ggsave("plot/04_CN_H_p.pdf", width = 8, height = 6)
...
CN_heat_ASD_H$groups <- cut(CN_heat_ASD_H$`p value`,              
                       breaks = c(1,0.1,0.05,0))
...
```{r}
ggplot(data=CN_heat_ASD_H,aes(phenotype1, phenotype2, fill=groups)) + 
    geom_tile() +
    theme_bw() +
  scale_fill_manual(values = c("darkblue","skyblue", "lightblue"), name = "p value", 
                  guide = guide_legend(reverse = TRUE)) +
